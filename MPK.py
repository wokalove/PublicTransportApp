...
class Window2(Window1):
    ...
    def __init__(self, master, number):
        ...
        self.student_label= tk.Label(self.master,text="Are you a student?\nType 'yes' or 'no'.",bg = "black",fg = "white")
        self.student_label.place(x=160,y=50)
        self.student_label.config(font=("Courier",20))
    
        
        self.student_place = tk.Entry(self.master,bd=20)
        self.student_place.place(x=220,y=130)
        
        self.connection_label= tk.Label(self.master,text="Connection",bg = "black",fg = "white")
        self.connection_label.place(x=180,y=200)
        self.connection_label.config(font=("Courier",30))        
        
        self.from_label= tk.Label(self.master,text="From:",bg = "black",fg = "white")
        self.from_label.place(x=100,y=280)
        self.from_label.config(font=("Courier",30))
        
        self.from_place = tk.Entry(self.master,bd=20)
        self.from_place.place(x=240,y=280)
        
        self.to_label= tk.Label(self.master,text="To:",bg = "black",fg = "white")
        self.to_label.place(x=140,y=380)
        self.to_label.config(font=("Courier",30))
        ...
        return_button = tk.Button(self.master,font=("Courier",12), text = "<--- Back",height = 2, width = 12,fg="red",command= lambda:self.return_window(self.master))
    ...
    def create_new_button(self, text, number, _class):
        ...
        b = tk.Button(self.master, text = text,height = 2, width = 20,fg="red",command= lambda:self.new_window(number, _class,self.get_user_answers()))
        b.config(font=("Courier",12))
        b.place(x=300,y=480)
        
    def show_lines(self,inp_from,inp_to,if_student):
        ''' Function which is called on button and displays all stops from connect_to_data function'''
        l = tk.Label(self.master, text ="I am looking for connection..",font=("Courier",12),bg="white", fg="black",wraplength=600)
        text=str(self.connect_to_data(inp_from,inp_to,if_student)).replace("]","").replace("[","")
        l.config( text = text)

        if text == []:
            l.config(text="We don't have such connection!:(",wraplength=300,font=("Courier",12))
        ...
        if line_number:
            ...
            with open('graf.json',encoding="utf8") as json_file:
                ...
                if all (inp not in new_dict for inp in (inp_from,inp_to)):
                    l = tk.Label(self.master, text ="We don't have such connection...",font=("Courier",12),bg="white", fg="black",wraplength=600)
                    l.place(x=300,y=130, anchor="center")
                else:
                    shortest_way = self.find_shortest_path(new_dict, inp_from, inp_to)
                    fullPath = self.path_with_correct_lines(shortest_way,new_dict)
                    self.display_bus_stops_indirect(shortest_way,fullPath,inp_from,inp_to,if_student)
    ...
    def find_stops(self,number,inp_from,inp_to,if_student):
        ...
        cost_lab = tk.Label(self.master, text = text,font=("Courier",12),bg="black", fg="white",wraplength=300)
    ...
    def bus_stops_display(self,busStops,inpFrom,inpTo):
        ...
        l = tk.Label(self.master, text = "Write down line of tram/bus!!!",font=("Courier",8),bg="white", fg="black")
    ...
    def making_graph_from_file_text(self,text):
        ''' Making new_dict from dictionary {nr_line: next_stops} from .json file
            new_dict are conections specific line between neighboring pairs of stops'''
        new_dict = {}
    ...
    def display_bus_stops_indirect(self,short_path,long_path,From,To,if_student):
        ''' Displaying path of indirect connections  ith removing duplicate lines'''
        ...
        for i in range(len(short_path)-1):
            ...
            if(already == 0):
                lines = np.intersect1d(long_path[short_path[i]], long_path[short_path[i+1]])
    ...
    def save_to_file_ONLY_ONCE():
        ...
        for line in line_numbers:
            ...
            json.dump( graph, open( 'graf.json', 'w' ,encoding='utf8'),ensure_ascii=False )
...
class Win3(Window2):
    def __init__(self, master, number,studentAns):
        ...
        self.label_found.config(font=("Courier",25),text="Found connections",bg="black", fg="white")
        self.label_found.pack(pady=10)

        b = tk.Button(self.master, text="Show Lines", command=lambda:self.show_lines(self.studentAns[1],self.studentAns[2],self.studentAns[0]),height=1,width=10,fg="red",bg="white")
        b.config(font=("Courier",12))
        b.pack(pady=2)

        self.label_type_line = tk.Label(self.master)
        self.label_type_line.config(font=("Courier",25),text="Type line from given above:",bg="black", fg="white")
        ...
        show_button = tk.Button(self.master, text = f"Show Stops",height = 1, width = 10,fg="red", command = lambda : self.find_stops(self.choose_line_place,self.studentAns[1],self.studentAns[2],self.studentAns[0]))
        show_button.place(x=400,y=210)
        show_button.config(font=("Courier",15))
        

        self.quit_button = tk.Button(self.master, text = f"Quit",height = 1, width = 10,fg="red", command = self.close_window)
        self.quit_button.configure(font=("Courier",12))
        self.quit_button.place(x=320,y=560)
        
        back_button = tk.Button(self.master,font=("Courier",12), text = "<--- Back",height = 1, width = 10,fg="red",command= lambda:self.return_window(self.master))
